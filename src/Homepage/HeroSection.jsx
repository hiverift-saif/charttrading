...
const HeroSection = () => {
  const { darkMode } = useTheme(); // darkMode state use ki
  ...
  return (
    // Background: Dark mein black, Light mein soft gray
    <div className={`md:min-h-screen w-full transition-colors duration-500 font-sans flex flex-col items-center justify-start md:justify-center px-4 pt-4 md:pt-0
      ${darkMode ? "bg-black text-bg-white" : "bg-white text-slate-900"}`}>
      
      {/* SECTION 1: ANIMATED CARD */}
      <div className="mt-2 mb-3 flex items-center justify-center">
        <div 
          key={currentAsset.name}
          className={`flex items-center gap-3 border rounded-full px-3 py-1.5 shadow-lg min-w-10 animate-fade-up cursor-pointer transition-all
            ${darkMode ? "bg-black border-[#403935] hover:bg-[#2e2724]" : "bg-white border-gray-200 hover:bg-gray-100"}`}
        >
          <img src={currentAsset.icon} alt={currentAsset.name} className="w-6 h-6 md:w-8 md:h-8" />
          <p className={`font-medium text-xs md:text-sm whitespace-nowrap ${darkMode ? "text-[#eceae9]" : "text-slate-800"}`}>
            {currentAsset.name}
          </p>
      ...
      <h1 className={`text-[32px] sm:text-[40px] md:text-[70px] leading-[1.1] md:leading-tight font-bold tracking-tight text-center w-full md:max-w-4xl mx-auto mb-5 font-nunito-custom transition-colors
        ${darkMode ? "text-[#cccac9]" : "text-slate-900"}`}>
      ...
      {/* 5. TRUSTPILOT RATING */}
      <div className={`flex items-center gap-2 text-xs md:text-sm font-medium py-4 md:py-5 ${darkMode ? "text-[#afadac]" : "text-slate-500"}`}>
      ...
      <div className="w-full flex justify-center mb-2 px-2 md:px-4">
        <div className={`relative w-full max-w-[1100px] backdrop-blur-md border rounded-2xl md:rounded-3xl p-2 md:p-4 shadow-2xl transition-colors
          ${darkMode ? "bg-white/5 border-white/10" : "bg-black/5 border-black/10"}`}>
          <div className={`relative w-full h-[200px] md:h-[420px] bg-black rounded-xl md:rounded-2xl overflow-hidden border 
