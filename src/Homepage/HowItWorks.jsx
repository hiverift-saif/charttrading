...
const HowItWorks = () => {
  ...
  return (
    <div className={`w-full md:py-16 py-5 px-6 font-sans transition-colors duration-500 
      ${darkMode ? "bg-black" : "bg-white"}`}>
      
      {/* Heading */}
      <div className="text-center mb-16">
        <h2 className={`text-3xl md:text-5xl font-bold mb-4 transition-colors
          ${darkMode ? "text-[#cccac9]" : "text-black"}`}>
          How It Works
        </h2>
        <p className={`text-sm md:text-base max-w-lg mx-auto transition-colors
          ${darkMode ? "text-[#afadac]" : "text-gray-600"}`}>
      ...
      <div className="relative max-w-4xl mx-auto">
        ...
        <div className={`absolute left-[20px] md:left-1/2 top-0 bottom-0 w-[2px] transform md:-translate-x-1/2 transition-colors
          ${darkMode 
            ? "bg-gradient-to-b from-orange-500/50 via-[#38312e] to-transparent" 
            : "bg-gradient-to-b from-orange-500/50 via-gray-200 to-transparent"}`}>
        </div>
          ...
          {steps.map((step, index) => (
            <div key={index} className={`relative flex items-center justify-between w-full ${index % 2 === 0 ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
              ...
              <div className="ml-12 md:ml-0 md:w-[45%]">
                <div className={`p-6 rounded-2xl shadow-xl transition-all group border
                  ${darkMode 
                    ? "bg-[#1a1614] border-[#38312e] hover:border-orange-500/30" 
                    : "bg-white border-gray-100 hover:border-orange-500/20 hover:shadow-2xl"}`}>
              ...
              <div className="absolute left-0 md:left-1/2 transform -translate-x-0 md:-translate-x-1/2 flex items-center justify-center">
                <div className={`w-10 h-10 rounded-full border-2 border-orange-500 flex items-center justify-center z-10 shadow-[0_0_15px_rgba(255,126,19,0.3)] transition-colors
                  ${darkMode ? "bg-black" : "bg-white"}`}>
