...
import API_CONFIG from '../config'; // Adjust path as per your project structure
...
if (response.ok) {
  ...
  console.log('doenodoeno',userData)
  localStorage.setItem('token', data.data.access_token);
  localStorage.setItem('user', JSON.stringify(userData));

  // Fix: MongoDB style ID के लिए _id use करो, fallback के साथ
  const savedUserId = userData._id || userData.id || userData.userId;

  if (!savedUserId) {
    setError("User ID not received from server. Contact support.");
    setIsLoading(false);
    return;
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black px-4 py-12 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#09090b]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl relative z-10">
        ...
        <button 
          type="button"
          className="w-full flex items-center justify-center gap-3 bg-[#18181b]/80 border border-white/10 text-white font-medium py-3 rounded-xl hover:bg-[#27272a]/80 hover:border-[#ffae34]/30 transition-all duration-300 mb-6 backdrop-blur-sm group"
          disabled={isLoading}
        >
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <span className="text-white/30 text-xs uppercase tracking-wider">Or login with email</span>
          <div className="h-px bg-white/10 flex-1"></div>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="space-y-1">
            ...
            <div className="relative group">
              ...
              <input 
                ...
                className={`w-full bg-[#18181b]/80 border rounded-xl px-11 py-3 text-white placeholder:text-white/20 focus:outline-none transition-all text-sm backdrop-blur-sm ${
                  errors.email 
                    ? 'border-red-500/50 bg-red-500/5' 
                    : 'border-white/10 focus:border-[#ffae34]/50 focus:ring-1 focus:ring-[#ffae34]/30 hover:border-white/20'
                }`} 
              />
              {errors.email && (
          ...
          <div className="space-y-1">
            ...
            <div className="relative group">
              ...
              <input 
                ...
                className={`w-full bg-[#18181b]/80 border rounded-xl px-11 py-3 text-white placeholder:text-white/20 focus:outline-none transition-all text-sm backdrop-blur-sm pr-12 ${
                  errors.password 
                    ? 'border-red-500/50 bg-red-500/5' 
                    : 'border-white/10 focus:border-[#ffae34]/50 focus:ring-1 focus:ring-[#ffae34]/30 hover:border-white/20'
                }`} 
              ...
              <button 
                ...
                className="absolute right-4 top-3.5 text-white/30 hover:text-[#ffae34] transition-colors"
              >
          ...
          <div className="flex justify-end pt-1">
            <a href="/forgot-password" className="text-xs text-white/50 hover:text-[#ffae34] transition-colors">
              Forgot password?
            </a>
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-white/10 text-center">
          <p className="text-white/40 text-sm">
            ...
            <a href="/signup" className="text-[#ffae34] font-medium hover:underline">Sign up free</a>
          </p>
        </div>
      </div>
    </div>
